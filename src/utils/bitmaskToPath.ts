...
export const bitmaskToPath = (
  ...
  if (options.width) {
    ...
    if (height % 1 !== 0) {
      throw new Error(
        `Invalid bitmask width. ${height} = ${bitmask.length} / ${width}`
      );
    }
  ...
  const edges = Array(edgeCount)
    .fill(0)
    .map(() => ({ x: 0, y: 0, next: undefined })) as Edge[];
  ...
  function UnionGroup(edge: Edge) {
    let itr;
    for (itr = edge.next; itr !== undefined && itr !== edge; itr = itr.next) {
      groups.delete(itr);
    }
    if (itr !== undefined) {
      groups.add(edge);
  ...
  for (const edge of groups) {
    ...
    for (let itr = edge.next; itr != edge; itr = itr?.next) {
      if (itr?.type == "H") {
